...
export const createTool = [
  ...
  async (req, res) => {
    try {
      ...
      if (isNaN(price) || price < 0) return res.status(400).json({ message: "Price must be a non-negative number" });
      if (!['Power Tools', 'Accessories'].includes(category)) return res.status(400).json({ message: "Invalid category" });
      ...
      if (existingTool) return res.status(400).json({ message: "Tool with this name already exists" });
      ...
      console.error(error);
      if (error.code === 11000) return res.status(400).json({ message: "Tool with this name already exists" });
